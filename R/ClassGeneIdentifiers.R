setClass("GeneIdentifiers", representation(GeneSet1="character", GeneSet2="character"))

setMethod(
	f= "sim", 
	signature= "GeneIdentifiers", 
	definition=function(object, params){
		...
		for (i in seq(along=object@GeneSet1)) {
			for (j in seq(along=object@GeneSet2)) {
				goids <- new("GOIdentifiers", GOSet1=GOS1[[i]], GOSet2=GOS2[[j]])
