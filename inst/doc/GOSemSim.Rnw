...
% require(cacheSweave); Sweave('GOSemSim.Rnw', driver=cacheSweaveDriver()); tools::texi2dvi("GOSemSim.tex", pdf=TRUE)
...
further analyzed viral microRNAs using this method  \cite{yu_new_2011}
and compared significant KEGG pathways regulated by different viruses'
microRNAs using \Rpackage{clusterProfiler}  \cite{yu2012}.
