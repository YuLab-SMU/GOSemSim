...
Please cite the following article when using `r Biocpkg("GOSemSim")`:
...
`r Biocpkg("GOSemSim")` package[@yu2010] is developed to compute
...
For IC-based methods, information of GO term is species specific. We need to calculate `IC` for all GO terms of a species before we measure semantic similarity. `r Biocpkg("GOSemSim")` support all organisms that have an `OrgDb` object available.
...
We can build query `OrgDb` online via `r Biocpkg("AnnotationHub")`. For example:
...
If organism is not supported by `r Biocpkg("AnnotationHub")`, user can use `r Biocpkg("AnnotationForge")` to build `OrgDb`.

Once we have `OrgDb`, we can build annotation data needed by `r Biocpkg("GOSemSim")` via `godata` function.
...
```{r}
goSim("GO:0004022", "GO:0005515", godata=hsGO, measure="Jiang")
goSim("GO:0004022", "GO:0005515", godata=hsGO, measure="Wang")
go1 = c("GO:0004022","GO:0004024","GO:0004174")
go2 = c("GO:0009055","GO:0005515")
mgoSim(go1, go2, godata=hsGO, measure="Wang", combine=NULL)
mgoSim(go1, go2, godata=hsGO, measure="Wang", combine="BMA")
```
...
```{r}
geneSim("241", "251", godata=hsGO, measure="Wang", combine="BMA")
mgeneSim(genes=c("835", "5261","241", "994"), 
         godata=hsGO, measure="Wang",verbose=FALSE)
mgeneSim(genes=c("835", "5261","241", "994"), 
       godata=hsGO, measure="Rel",verbose=FALSE)
```
...
mgeneSim(genes, godata=hsGO2, measure="Wang", combine="BMA", verbose=FALSE)
```
...
clusterSim(gs1, gs2, godata=hsGO, measure="Wang", combine="BMA")
...
mclusterSim(clusters, godata=hsGO, measure="Wang", combine="BMA")
```

# Applications

`r Biocpkg("GOSemSim")` was cited by more than [200 papers](https://scholar.google.com.hk/scholar?oi=bibs&hl=en&cites=9484177541993722322,17633835198940746971,18126401808149291947) and had been applied to many research domains, including:
