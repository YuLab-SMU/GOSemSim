...
mgoSim(go1, go2, godata=d, measure="Wang", combine=NULL)
mgoSim(go1, go2, godata=d, measure="Wang", combine="BMA")
```
...
```{r}
geneSim("241", "251", godata=d, measure="Wang", combine="BMA")
mgeneSim(genes=c("835", "5261","241", "994"), 
       godata=d, measure="Wang",verbose=FALSE)
```
...
clusterSim(gs1, gs2, godata=d, measure="Wang", combine="BMA")

x <- org.Hs.egGO
hsEG <- mappedkeys(x)
set.seed <- 123
clusters <- list(a=sample(hsEG, 20), b=sample(hsEG, 20), c=sample(hsEG, 20))
mclusterSim(clusters, godata=d, measure="Wang", combine="BMA")
```
